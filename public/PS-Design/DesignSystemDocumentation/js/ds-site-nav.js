(function () {
  ...
  var SITE_NAV_SHELL_CLASS = "site-navbar-shell";

  if (document.querySelector("." + SITE_NAV_SHELL_CLASS)) {
    return;
  }

  var SITE_LANG_KEY = "site_lang";
  var DOC_LANG_KEY = "ps-lang";
  ...
  function getLanguage() {
    var siteLang = window.localStorage.getItem(SITE_LANG_KEY);
    if (siteLang === "ar" || siteLang === "en") {
      return siteLang;
    }

    var docsLang = window.localStorage.getItem(DOC_LANG_KEY);
    if (docsLang === "ar" || docsLang === "en") {
      return docsLang;
    }
  ...
  function setLanguage(lang) {
    ...
    window.localStorage.setItem(SITE_LANG_KEY, normalized);
    window.localStorage.setItem(DOC_LANG_KEY, normalized);
  }
  ...
  function getLabels(language) {
    ...
    return {
      links: {
        ...
        work: isArabic ? "الأعمال" : "Work",
        caseStudy: isArabic ? "دراسة حالة" : "Case Study",
        casebook: isArabic ? "نظام التصميم" : "Design System",
        blog: isArabic ? "المدونة" : "Blog",
        contact: isArabic ? "تواصل" : "Contact"
      },
      brand: "UX & DesignOps Lead",
      logoAlt: isArabic ? "شعار عمر مختار الشخصي" : "Omar Mokhtar personal logo",
      navLabel: isArabic ? "القائمة الرئيسية" : "Main navigation",
      menuOpen: isArabic ? "افتح القائمة" : "Open menu",
  ...
  function isLinkActive(linkHref, path) {
    ...
    if (
      ...
      path.indexOf("/PS-Design/DesignSystemDocumentation/") === 0
    ) {
      return true;
    }
  ...
  function renderNavLinks(language, currentPath, isMobile) {
    var labels = getLabels(language);
    var listClass = isMobile ? "navbar-nav navbar-nav--mobile" : "navbar-nav";
    ...
    navLinks.forEach(function (link) {
      ...
      html +=
        '<li class="nav-item">' +
        '<a href="' + link.href + '" class="nav-link' + (active ? " active" : "") + '"' +
  ...
  function render(language) {
    ...
    var desktopBrandSection =
      '<div class="navbar__inner">' +
      '<div class="d-flex align-items-center brand" data-aos="fade-right" data-aos-duration="2000">' +
      '<a href="/" class="navbar-brand" aria-label="' + labels.brandAria + '">' +
      '<img alt="' + labels.logoAlt + '" src="/images/mylogo.png" width="48" height="48" loading="eager" />' +
      "</a>" +
      '<div class="navtitle">' + labels.brand + "</div>" +
      "</div>" +
      '<button type="button" class="navbar__toggle" aria-controls="mobileNav" aria-expanded="false" aria-label="' + labels.menuOpen + '">' +
      buildMenuIcon() +
      "</button>" +
      "</div>";

    var navHtml =
      '<div class="' + SITE_NAV_SHELL_CLASS + '">' +
      '<div class="navBg navBg--glass">' +
      '<div class="container">' +
      '<nav class="navbar navbar-expand-lg p-3 d-flex justify-content-between ' + (isArabic ? "rtl-navbar" : "") + '" dir="' + (isArabic ? "rtl" : "ltr") + '">' +
      (isArabic ? desktopNavLinksSection + desktopBrandSection : desktopBrandSection + desktopNavLinksSection) +
      "</nav>" +
      "</div>" +
      "</div>" +
      '<div class="nav-shell">' +
    ...
    var siteNavShell = wrapper.firstElementChild;
    document.body.insertBefore(siteNavShell, document.body.firstChild || null);

    var liveNavBg = siteNavShell.querySelector(".navBg");
    var liveNavShell = siteNavShell.querySelector(".nav-shell");
    var toggleButton = siteNavShell.querySelector(".navbar__toggle");
    var backdrop = siteNavShell.querySelector(".navbar__backdrop");
    var mobileNav = siteNavShell.querySelector("#mobileNav");
    var langButton = siteNavShell.querySelector(".lang-btn");
    ...
    function updateNavState() {
      ...
      var nextIsSolid = offset > SCROLL_THRESHOLD;

      liveNavBg.classList.toggle("navBg--solid", nextIsSolid || isMenuOpen);
      liveNavBg.classList.toggle("navBg--glass", !nextIsSolid && !isMenuOpen);
    }
    ...
    function setMenu(open) {
      ...
      if (liveNavShell) {
        liveNavShell.classList.toggle("is-menu-open", isMenuOpen);
      }
      ...
      if (mobileNav) {
        ...
        mobileNav.setAttribute("aria-hidden", isMenuOpen ? "false" : "true");
      }

      if (toggleButton) {
        toggleButton.setAttribute("aria-expanded", isMenuOpen ? "true" : "false");
        toggleButton.setAttribute("aria-label", isMenuOpen ? labels.menuClose : labels.menuOpen);
        toggleButton.innerHTML = isMenuOpen ? buildCloseIcon() : buildMenuIcon();
      }
    ...
    if (langButton) {
      langButton.addEventListener("click", function () {
        var nextLang = language === "en" ? "ar" : "en";
