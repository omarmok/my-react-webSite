...
const Nav = ({
  ...
  const buildLinkProps = (link, { closeMoreOnClick = false, role } = {}) => {
    ...
    const props = {
      ...
      onClick: closeMoreOnClick ? handleClick : undefined,
      role: role ?? undefined,
    };
  ...
  const moreToggleAria = t("nav.moreMenuAria") ?? "More navigation links";

  const handleMoreToggle = () => {
    setIsMoreOpen((previous) => !previous);
  };
    ...
    <ul className="navbar-nav" role="menubar">
      {mainNavLinks.map((link) => (
        <li className="nav-item" key={link.key} role="none">
          <Link href={link.href} {...buildLinkProps(link, { role: "menuitem" })}>
            {t(`nav.links.${link.key}`)}
          </Link>
        </li>
      ))}

      <li className="nav-item nav-item--more" ref={moreMenuRef} role="none">
        <button
          type="button"
          ref={moreButtonRef}
  ...
  const navBgClassName = `navBg ${navIsSolid || isMoreOpen ? "navBg--solid" : "navBg--glass"}`;
      ...
      <div className={navBgClassName}>
        <div className="container">
          ...
          >
            ...
            >
              <div className="navbar__links-area">{renderDesktopNavLinks()}</div>
              <div className="navbar__lang-area">
